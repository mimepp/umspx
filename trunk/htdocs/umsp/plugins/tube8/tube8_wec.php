...
include_once($_SERVER[DOCUMENT_ROOT] . '/umsp/funcs-config.php');
	...
	if ((defined('WECVERSION')) && (WECVERSION >= 3)) {
		...
		$wec_options[$pluginInfo['id']] = array(
		 ...
		 $wec_options['TUBE8_MAX_PAGE_DEPTH'] = array(
			'configname'	=> 'TUBE8_MAX_PAGE_DEPTH',
		...
		if(_pluginGetT8ShowData($cats,$catshtml))
		{
			...
			foreach(array('Gay','Shemale') as $cat)
			{
				if(in_array($cat,$availvvv)==false)
				{
					$availvvv[] = $cat;
					$availnnn[] = $cat;
				}
			}
			$wec_options['TUBE8_EXCLUDE_SHOWS'] = array(
				'configname'	=> 'TUBE8_EXCLUDE_SHOWS',
